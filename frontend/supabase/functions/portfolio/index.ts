...
serve(async (req) => {
  ...
  try {
    ...
    const { action, filters, configName } = await req.json()
    ...
    switch (action) {
      case 'getPortfolioTrades':
        ...
        for (const trade of trades || []) {
          ...
          const closeTransactions = trade.transactions.filter((t: any) =>
            t.transaction_type === 'CLOSE' || t.transaction_type === 'TRIM'
          )
          const openTransactions = trade.transactions.filter((t: any) =>
          ...
          if (trade.average_exit_price) {
            ...
            totalRealizedPL = openTransactions.reduce((sum: number, t: any) => 
              sum + ((parseFloat(t.amount) - trade.average_price) * parseFloat(t.size)), 0
            ) * -1
          }
      ...
      case 'getMonthlyPL':
        ...
        const filteredStrategies = monthlyStrategies?.filter(strategy => 
...
function calculateStrategyPL(strategy: any): [number, number] {
  ...
  const totalExitValue = closeTransactions.reduce((sum: number, t: any) => 
    sum + (t.net_cost * parseFloat(t.size)), 0
  )
  const totalExitSize = closeTransactions.reduce((sum: number, t: any) => 
    sum + parseFloat(t.size), 0
  )
...
function createTradeOneliner(trade: any): string {
  const parts = []
...
function createStrategyOneliner(strategy: any): string {
